...
use core\notification;
use quiz_datawarehouse\local\table\backend_list;
...
class backend_controller {
    ...
    protected function set_external_page() {
        admin_externalpage_setup('quiz_datawarehouse/backend');
    }
     ...
     *
     * @return \quiz_datawarehouse\backend
     */
    ...
    protected function get_create_button_text() : string {
        return get_string('addbackend', 'quiz_datawarehouse');
    }

    /**
     * Returns form for the record.
     *
     * @param \quiz_datawarehouse\backend|null $instance
     *
     * @return \quiz_datawarehouse\local\form\backend
     */
    protected function get_form($instance) : \quiz_datawarehouse\local\form\backend {
        global $PAGE;

        return new \quiz_datawarehouse\local\form\backend($PAGE->url->out(false), ['persistent' => $instance]);
    }
    ...
    protected function get_view_heading() : string {
        return get_string('managebackends', 'quiz_datawarehouse');
    }
    ...
    protected function get_new_heading() : string {
        return get_string('newbackend', 'quiz_datawarehouse');
    }
    ...
    protected function get_edit_heading() : string {
        return get_string('editbackend', 'quiz_datawarehouse');
    }
    ...
    public static function get_base_url() : string {
        return '/mod/quiz/report/datawarehouse/backend.php';
    }
        ...
        if ($form->is_cancelled()) {
            ...
            try {
                if (empty($data->id)) {
                    ...
                    \quiz_datawarehouse\event\backend_created::create_strict(
                        $persistent,
                    ...
                    \quiz_datawarehouse\event\backend_updated::create_strict(
                        $instance,
                        \context_system::instance()
                    )->trigger();
    ...
    protected function delete($id) {
        ...
        if ($instance->can_delete()) {
            ...
            \quiz_datawarehouse\event\backend_deleted::create_strict(
                $id,
                \context_system::instance()
            )->trigger();
    ...
    protected function view() {
        ...
        $PAGE->requires->js_call_amd('quiz_datawarehouse/managebackends', 'setup');
     ...
     *
     * @param \quiz_datawarehouse\backend $backend The backend persistent object.
     */
    private function trigger_enabled_event(backend $backend) {
        $eventstring = ($backend->get('enabled') == 0 ? 'disabled' : 'enabled');

        $func = '\quiz_datawarehouse\event\backend_' . $eventstring;
        $func::create_strict(
            $backend,
            \context_system::instance()
        )->trigger();
